...
const appointment = {
  create: async (data) => {
    const sql = 'INSERT INTO appointment (userId, userName, fees, status, treatmentId, timing, email, phone, date, message, form, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())';
    try {
      const [results] = await db.execute(sql, [data.userId,data.userName,data.fees,data.status,data.treatmentId,data.timing,data.email,data.phone,data.date,data.message,data.form]);
